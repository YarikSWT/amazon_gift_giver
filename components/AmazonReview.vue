<template>
  <section name="review" class="review">
    <div class="container">
      <div class="content text-center my-5">
        <h1 class="font-weight-bolder">Thanks for your feedback!</h1>
        <h1 class="font-weight-bolder my-4">
          In the meantime, would you like to share your feedback to others?
        </h1>
      </div>
      <div class="content my-5">
        <p>
          Please take a moment to share your experience with others by clicking
          on the orange button and posting your review on Amazon. This only
          takes a second, and will help other customers know more about the
          product!
        </p>
      </div>
      <div class="row">
        <div class="col-lg-6 col-mg-6 col-12">
          <el-input
            v-model="review"
            class="mb-3"
            type="textarea"
            style="min-height: 150px; height: 150px;"
            :rows="6"
            :autosize="{ minRows: 2, maxRows: 4 }"
            placeholder="Please input"
          >
          </el-input>
        </div>
        <div class="col-lg-6 col-mg-6 col-12">
          <div class="amazon_click pt-0 pt-lg-3 pt-mt-3 w-100">
            <a
              :href="$store.state.inputs.reviewLink"
              target="_blank"
              class="btn-amazon"
            >
              Click here to post your review!
            </a>
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </div>
        </div>
        <div class="сol-12 d-flex mt-5 justify-content-center w-100">
          <el-button
            type="success"
            class="btn btn-green order-btn"
            :disabled="review.length === 0"
            @click="onSubmit"
          >
            Continue to choose the gift
          </el-button>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: 'AmazonReview',
  data() {
    return {
      form: {
        review: '',
      },
    }
  },
  computed: {
    review: {
      get() {
        return this.$store.state.inputs.review
      },
      set(newValue) {
        this.form.review = newValue
        this.$store.commit('setInput', { field: 'review', data: newValue })
      },
    },
  },
  methods: {
    onSubmit() {
      this.$emit('nextStep', this.review)
    },
  },
}
</script>

<style>
.amazon_click {
  position: relative;
  background-color: rgb(230, 231, 233);
  min-height: 200px;
  height: 200px;
}
.amazon_click img {
  /* position: absolute; */
  z-index: 1;
  right: 0;
  width: 200px;
}

.review h1,
h2,
p {
  color: #657380;
}
.btn-amazon {
  margin: 5px;
  /* position: absolute; */
  z-index: 2;
  color: #fff !important;
  text-decoration: none;
  font-family: Arial;
  position: relative;
  font-size: 90%;
  font-weight: bold;
  text-align: center;
  cursor: pointer;
  border: none;
  border-radius: 0.5em;
  padding: 0.5em;
  background-color: #f90;
  box-shadow: 1px 1px 2px #888888;
  background-image: -webkit-linear-gradient(
    bottom,
    #e80 0%,
    #f90 60%,
    #fb4 100%
  );
  background-image: linear-gradient(to top, #ff9900 60%, #ffbb44 100%);
}

.btn-amazon {
  margin-top: 40px;
  margin-left: 30px;
}

@media (min-width: 320px) and (max-width: 760px) {
  .amazon_click {
    margin-top: 30px;
    display: flex;
    align-items: center;
    flex-direction: column;
  }

  .amazon_click .btn-amazon {
    margin-top: 10px;
    margin-left: 10px;
  }

  h1 {
    font-size: 20px;
  }
}
</style>
